...
def raster_to_json(line, fp_id, print_door_warning):
    ...
    for w_i in range(len(walls)):
        edges.append([
            ((walls[w_i][0] - min_x) / lenx),
            ((walls[w_i][1] - min_y) / leny),
            ((walls[w_i][2] - min_x) / lenx),
            ((walls[w_i][3] - min_y) / leny),
            walls[w_i][5],
            walls[w_i][8]
        ])
        if (walls[w_i][6] == -1):
    ...
    # Save JSON file using the provided fp_id
    output_path = Path("rplan_json") / f"{fp_id}.json"
...
def parse_args():
    ...
    parser.add_argument("--id", type=int, required=True, help="ID of the floorplan")  # Add --id argument
    return parser.parse_args()

def main():
    args = parse_args()
    line = args.path
    fp_id = args.id  # Use the provided ID

    try:
        raster_to_json(line, fp_id, print_door_warning=False)
    except (AssertionError, ValueError, IndexError) as e:
        # Also use the provided ID for the failed marker
        failed_path = Path("failed_rplan_json") / str(fp_id)
